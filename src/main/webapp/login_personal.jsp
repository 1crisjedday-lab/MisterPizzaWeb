        ...
        <div class="w-full max-w-md z-10">
            ...
            <div class="bg-zinc-900/80 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-zinc-800">
                ...
                <% String error=request.getParameter("error"); if (error !=null) { String
                    msg="Ocurrió un error en el acceso." ; if ("credenciales".equals(error)) {
                    msg="Acceso denegado. Credenciales no válidas para esta área de personal." ; } else if
                    ("bd".equals(error)) { msg="Error de conexión con la base de datos." ; } else if
                    ("dashboard".equals(error)) { msg="Sesión no válida o error al cargar el dashboard." ; } %>
                    <script>
                    ...
                    <% } %>
                        ...
                        <form action="/LoginPersonalServlet" method="POST" class="space-y-6">
                            ...
                            <div>
                                <label class="block text-zinc-400 text-xs font-bold uppercase tracking-widest mb-2"
                                    for="password">Contraseña de Acceso</label>
                                <div class="relative">
                                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-500"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button type="submit"
                                class="w-full bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest py-3 px-4 rounded-md shadow-[0_0_15px_rgba(220,38,38,0.4)] transition-all">
                                Autenticar
                            </button>
                        </form>
            </div>
            ...
            <a href="index.jsp"
                class="block text-center w-full mt-6 text-zinc-500 hover:text-white font-bold transition-colors text-sm">
